...
{% if settings.home_order_position_1 == 'news_banners' and has_news_banner %}
    {% for slide in settings.banner_news %}
        {% set slide_image = slide.image | static_url %}
        {% if loop.first %}
            <link rel="preload" as="image" href="{{ slide_image | settings_image_url('original') }}" imagesrcset="{{ slide_image | settings_image_url('large') }} 480w, {{ slide_image | settings_image_url('huge') }} 640w, {{ slide_image | settings_image_url('original') }} 1024w">
        {% endif %}
    {% endfor %}
{% endif %}
